...
write.csv(tbl_nonull,"tbl_nonull.csv")
...
class(tbl_nonull$Date)
weekend1 <- c("sobota","neděle")
#weekdays(tbl_nonull$Date)
tbl_nonull$Wday <- factor((weekdays(tbl_nonull$Date) %in% weekend1),levels=c(TRUE,FALSE),labels=c("weekend","weekday"))
tbl_nonull$day <- weekdays(tbl_nonull$Date) 
head(tbl_nonull)
tail(tbl_nonull)
tbl_nonull[]
#tbl_nonull$Wday=="weekday", %in% weekend1,levels=c(TRUE,FALSE),labels=c("weekend","weekday"))
